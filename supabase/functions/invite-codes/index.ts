...
async function listInviteCodes(req: Request): Promise<Response> {
  ...
  if (hubId && !isSuper) {
    // Check requester roles for that hub using shared helper
    try {
      const { roles } = await fetchUserRoles(user.id);
      const allowed = (roles || []).some((r: any) =>
        r.hub_id === hubId && ["admin", "hub_manager"].includes(r.role)
      );
      if (!allowed) {
        return errorResponse(
          "FORBIDDEN",
          "Not authorized to view invites for this hub",
          403,
        );
      }
    } catch (e) {
  ...
  try {
    ...
    return errorResponse(
      ...
      "Service role key not configured for list operation",
      500,
    );
  }
  ...
  try {
    ...
    const probe = await serviceClient.from("invite_codes").select("hub_id")
      .limit(1).maybeSingle();
  ...
  if (!hubId) {
    if (!isSuper) {
      try {
        const { roles } = await fetchUserRoles(user.id);
        const adminHubIds = (roles || []).filter((r: any) =>
          ["admin", "hub_manager"].includes(r.role) && r.hub_id
        ).map((r: any) => r.hub_id).filter(Boolean);
  ...
  let query = serviceClient.from("invite_codes").select("*").order(
    "created_at",
    { ascending: false },
  ).range(start, end);
  if (!hasHubIdColumn) {
    ...
    if (hubId) {
      return errorResponse(
        "BAD_REQUEST",
        "This deployment does not support hub-scoped invites",
        400,
      );
    }
...
async function createInviteCode(req: Request): Promise<Response> {
  ...
  const body = await validateBody(req, createInviteCodeSchema) as z.infer<
    typeof createInviteCodeSchema
  >;
  ...
  try {
    const functionsDomain = (Deno.env.get("SUPABASE_URL") || "").replace(
      "https://",
      "",
    ...
    const confirmationBase = Deno.env.get("SITE_CONFIRMATION_URL") ||
      ...
      Deno.env.get("VITE_SITE_URL") || Deno.env.get("VITE_APP_URL") || "";
    let confirmationUrl = "";
    if (confirmationBase) {
    ...
    (async () => {
      try {
        ...
        if (!resp.ok) {
          ...
          console.error(
            "[invite-validate] send-invite-confirmation failed",
            resp.status,
            b,
          );
        } else {
        ...
        console.error(
          "[invite-validate] error invoking send-invite-confirmation",
          e,
        );
      }
    })();
...
async function sendInvite(req: Request): Promise<Response> {
  ...
  }).refine((v: { code?: string; email?: string }) => !!(v.code || v.email), {
    message: "Either code or email must be provided",
  });
  const body = await validateBody(req, sendInviteSchema) as {
    code?: string;
    email?: string;
    subject?: string;
  };
  ...
  try {
    console.debug(
      "[invite-codes] sendInvite called by user=",
      user?.id,
      "payload=",
  ...
  if (body.code) {
    const { data, error } = await getServiceRoleClient().from("invite_codes")
      .select("*").eq("code", body.code).maybeSingle();
    if (error) throw error;
    inviteRow = data;
  } else if (body.email) {
    const { data, error } = await getServiceRoleClient().from("invite_codes")
      .select("*").eq("invited_email", body.email).order("created_at", {
        ascending: false,
      }).limit(1);
    if (error) throw error;
  ...
  if (!inviteRow.invited_email) {
    console.error(
      "[invite-codes] sendInvite: invite found but invited_email is missing",
      { code: body.code, id: inviteRow.id },
    );
    return errorResponse(
      "NO_EMAIL",
      "Invite does not have an email to send to",
      400,
    );
  }
  ...
  const payload = {
    ...
    subject: body.subject ||
      `You were invited to join ${Deno.env.get("SITE_NAME") || "Jenga Biz"}`,
  };
...
async function validateInviteCode(req: Request): Promise<Response> {
  const url = new URL(req.url);
  const { code } = validateQuery(url, validateInviteCodeQuerySchema) as z.infer<
    typeof validateInviteCodeQuerySchema
  >;
  ...
  try {
    console.debug(
      "[invite-validate] runtime SUPABASE_URL=",
  ...
  try {
    if (data) {
      console.debug("[invite-validate] db found invite:", {
        code: data.code,
      ...
      console.debug(
        "[invite-validate] db did not find invite for code:",
        code,
        "as of",
        nowIso,
      );
...
async function consumeInviteCode(req: Request): Promise<Response> {
  ...
  const { code, user_id } = await validateBody(
    req,
    consumeInviteCodeSchema,
  ) as z.infer<typeof consumeInviteCodeSchema>;
  ...
  if (invite.account_type === "organization") {
    try {
      ...
      const creatorIsSuper = (creatorRoles || []).some((r: any) =>
        r.role === "super_admin"
      );
      if (creatorIsSuper) {
        try {
          ...
          if (rpcErr) {
            if ((rpcErr as any).code !== "23505") {
              console.error(
                "Failed to auto-assign admin role for organization invite (rpc):",
                rpcErr,
              );
            } else {
              console.debug(
                "service_add_user_role: admin role already exists for user",
                { user_id, invite_code: code },
              );
            }
          } else {
            console.debug(
              "Auto-assigned admin role to user from organization invite (rpc)",
              { user_id, invite_code: code },
            );
          }
        } catch (e) {
          console.error(
            "Error calling service_add_user_role RPC for org invite auto-assign:",
            e,
          );
        }
      }
      ...
      console.error(
        "Error checking creator roles for org invite auto-assign:",
        e,
      );
    ...
    (async () => {
      try {
        ...
        if (!resp.ok) {
          ...
          console.error(
            "[invite-codes] send-signup-confirmation failed",
            resp.status,
            txt,
          );
        } else {
        ...
        console.error(
          "[invite-codes] error invoking send-signup-confirmation",
          e,
        );
      }
    })();
  ...
  try {
    ...
    if (!allowed) {
      ...
      if (creatorHubId) {
        ...
        const hasHubAdmin = (myRoles || []).some((r: any) =>
          (r.role === "hub_manager" || r.role === "admin") &&
          r.hub_id === creatorHubId
        );
    ...
    return errorResponse(
      "FORBIDDEN",
      "Not authorized to delete this invite",
      403,
    );
  }
